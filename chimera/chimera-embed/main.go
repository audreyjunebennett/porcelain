...
func exitCodeForError(err error) int {
	var exitErr *wruntime.ExitError
...
func run(args []string) error {
	cfg, err := config.Parse(args, config.BuildInfo{Version: version, Commit: commit, Date: date})
	if err != nil {
	...
	backend := &adapter.LlamaServer{Cfg: cfg, Host: host, Port: port}
