...
// LlamaServer adapts llama-server embedding mode to the shared wrapper runtime.
...
func (a *LlamaServer) BackendName() string { return naming.ProductLlamaServerBinName }

// ChildLogWriter normalizes supervised chimera-embed output for operator logs.
func ChildLogWriter(dst io.Writer) io.Writer { return embedline.NewWriter(dst) }

func WrapUpstreamLine(raw string) string { return string(embedline.NormalizePayload(raw)) }
