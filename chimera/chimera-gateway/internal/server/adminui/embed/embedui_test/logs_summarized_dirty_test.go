...
func loadSummarizedDirtyRoutingCtx(t *testing.T, vm *goja.Runtime) {
	...
	evalJS(t, vm, logsUIPath(t, "app", "summarizedDirtyRouting.js"))
	_, err := vm.RunString(`
		var deps = {
...
func TestSummarizedDirtyRouting_conversationAndService(t *testing.T) {
	...
	fn, ok := goja.AssertFunction(vm.Get("ChimeraLogs").ToObject(vm).Get("Summarized").ToObject(vm).Get("dirtyTargetsForEntry"))
	if !ok {
		t.Fatal("missing ChimeraLogs.Summarized.dirtyTargetsForEntry")
	}
	...
	convID, err := vm.RunString(`ChimeraLogs.Summarized.conversationCardIdForPrincipalAndCid("tenant-a", "conv-1", ChimeraLogs.strHash)`)
	if err != nil {
		t.Fatal(err)
	}
	foundConv := false
	...
	svcID, err := vm.RunString(`ChimeraLogs.Summarized.serviceCardIdForBucketKey("chimera-gateway", ChimeraLogs.strHash)`)
	if err != nil {
		t.Fatal(err)
	}
	foundSvc := false
...
func TestSummarizedDirtyRouting_requestIdCorrelation(t *testing.T) {
	...
	convID, err := vm.RunString(`ChimeraLogs.Summarized.conversationCardIdForPrincipalAndCid("tenant-b", "conv-b", ChimeraLogs.strHash)`)
	if err != nil {
		t.Fatal(err)
	}
	found := false
...
func TestSummarizedDirtyRouting_brokerRelayBucketsToBrokerCard(t *testing.T) {
	...
	cardIds := v.ToObject(vm).Get("cardIds").Export().([]any)
	brokerID, err := vm.RunString(`ChimeraLogs.Summarized.serviceCardIdForBucketKey("chimera-broker", ChimeraLogs.strHash)`)
	if err != nil {
		t.Fatal(err)
	}
	found := false
