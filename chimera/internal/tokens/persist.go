...
func AppendToken(path, label string) (plainToken, tenantID string, err error) {
	...
	if raw, rerr := os.ReadFile(path); rerr == nil && len(strings.TrimSpace(string(raw))) > 0 {
		if uerr := yaml.Unmarshal(raw, &doc); uerr != nil {
			return "", "", fmt.Errorf("parse existing api-keys yaml: %w", uerr)
		}
	}
	appendCredentialRow(&doc, plainToken, tenantID, strings.TrimSpace(label))
...
func ListTokenMeta(path string) ([]TokenMeta, error) {
	...
	var out []TokenMeta
	appendUnique := func(secret, tenantID, label string, idx int) {
		if !rowIsValidSecret(secret, tenantID) {
			return
		}
		for _, existing := range out {
			if existing.Token == strings.TrimSpace(secret) {
				return
			}
		}
	...
	for i, row := range doc.APIKeys {
		appendUnique(row.Secret, row.TenantID, row.Label, i)
	}
	return out, nil
}

// RemoveTokenAt removes the row at doc.APIKeys[index] and rewrites the file atomically.
func RemoveTokenAt(path string, index int) error {
	...
	var metas []TokenMeta
	appendUnique := func(secret, tenantID, label string) {
		if !rowIsValidSecret(secret, tenantID) {
			return
		}
	...
	for _, row := range doc.APIKeys {
		appendUnique(row.Secret, row.TenantID, row.Label)
	}
	if index >= len(metas) {
		return fmt.Errorf("token index out of range")
	}
	target := strings.TrimSpace(metas[index].Token)
	if target == "" {
		return fmt.Errorf("token index out of range")
	}
	filterAPIKeys := doc.APIKeys[:0]
	for _, row := range doc.APIKeys {
		if strings.TrimSpace(row.Secret) == target {
			continue
		}
		filterAPIKeys = append(filterAPIKeys, row)
	}
	doc.APIKeys = filterAPIKeys
	out, err := yaml.Marshal(&doc)
	if err != nil {
		return err
	}
