...
import (
	...
	"github.com/lynn/porcelain/chimera/chimera-vectorstore/internal/adapter"
...
func run(args []string) error {
	...
	qdrantAdapter := &adapter.Qdrant{Cfg: cfg, Host: host, Port: port}

	rootCtx, stop := signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM)
	defer stop()
	return wruntime.Run(rootCtx, wruntime.Config{
		Component:              contract.ComponentVectorstore,
		BackendMode:            "binary",
		Listen:                 cfg.Listen,
