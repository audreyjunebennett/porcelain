...
|----------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|---------------|
| [Product naming](#product-naming)                                                                  | Layered names in docs, UI, and startup logs with naming contracts ([`plans/archive/v0-3-naming-migration.md`](plans/archive/v0-3-naming-migration.md)) | `done`        |
...
| [`plans/archive/chimera-gateway-package-boundaries.md`](plans/archive/chimera-gateway-package-boundaries.md)         | Admin UI / operator API package split; shared DTOs                                                                                     | `done`        |
| [`plans/archive/chimera-gateway-refactor.md`](plans/archive/chimera-gateway-refactor.md)                             | Gateway naming clarity; logs UI modularization train                                                                                   | `done`        |
| [`plans/archive/adminui-filesystem-dev-mode.md`](plans/archive/adminui-filesystem-dev-mode.md)                       | Optional `CHIMERA_ADMINUI_ROOT` disk serving for embed UI dev                                                                          | `done`        |
| [`plans/archive/embedui-component-gallery.md`](plans/archive/embedui-component-gallery.md)                           | Static component gallery paths and upkeep                                                                                              | `done`        |
| [`plans/archive/embedui-component-system.md`](plans/archive/embedui-component-system.md)                             | Reusable embed UI primitives and module split                                                                                          | `done`        |
| [`plans/archive/embedui-event-log-panel.md`](plans/archive/embedui-event-log-panel.md)                               | Per-card event log layout and interaction                                                                                              | `done`        |
| [`plans/archive/embedui-logs-workspaces-merge.md`](plans/archive/embedui-logs-workspaces-merge.md)                   | Unify logs and workspace indexers in embed UI                                                                                          | `done`        |
| [`plans/archive/embedui-theme-styleguide.md`](plans/archive/embedui-theme-styleguide.md)                             | Theme tokens and static styleguide                                                                                                     | `done`        |
| [`plans/archive/locus-desktop-supervisor-contract.md`](plans/archive/locus-desktop-supervisor-contract.md)           | Desktop ↔ supervisor process and readiness contract                                                                                    | `done`        |
| [`plans/archive/vectorstore-broker-wrapper-hard-cut.md`](plans/archive/vectorstore-broker-wrapper-hard-cut.md)       | **chimera-vectorstore** / **chimera-broker** wrapper binaries and supervisor cutover                                                   | `done`        |
| [`plans/archive/v0-3-naming-migration.md`](plans/archive/v0-3-naming-migration.md)                                   | Product naming hard-cut execution                                                                                                      | `done`        |
| [`plans/archive/logs-ui-page-data-refreshing.md`](plans/archive/logs-ui-page-data-refreshing.md)                     | Summarized logs feed: interaction-safe rebuilds, card patches, view model                                                              | `done`        |
| [`plans/archive/embedui-operator-settings-routes.md`](plans/archive/embedui-operator-settings-routes.md)             | App shell at `/ui`, settings page rename, single gallery; drop legacy routes and deep links                                            | `done`        |
| [`plans/archive/context-window-admission.md`](plans/archive/context-window-admission.md)                             | Context window admission on chat path. Capture provider model context limits to enabled proper model router fallbacks                  | `done`        |
| [`plans/archive/supervisor-info-log-trim.md`](plans/archive/supervisor-info-log-trim.md)                             | Refine the logs even further to reduce service start up and vectorstore erroring. Lowering heartbeat logs to DEBUG                     | `done`        |
| [`plans/archive/embedui-dynamic-provider-cards.md`](plans/archive/embedui-dynamic-provider-cards.md)                 | Settings provider cards: catalog, Add provider picker, hide usage/log until configured                                                 | `done`        |
...
**Companion docs:** `[chimera.plan.md](chimera.plan.md)`, `[configuration.md](configuration.md)`, `[`plans/archive/indexer.md`](plans/archive/indexer.md)`, `[`plans/archive/v0-3-naming-migration.md`](plans/archive/v0-3-naming-migration.md)` (product naming execution), `[`plans/archive/virtual-models-operator.md`](plans/archive/virtual-models-operator.md)` (virtual models execution), plus implementation plans in [Related plans](#related-plans) (gateway/embed UI refactor, supervisor contract, wrapper hard cut).

Authoritative **architecture and numbered requirements** remain in `[chimera.plan.md](chimera.plan.md)` unless this plan explicitly revises them. **Indexer** **Phase 3** in `[`plans/archive/indexer.md`](plans/archive/indexer.md)` (e.g. scoped overrides, headers) is **not** the same shipping train as **gateway desktop v0.3**; cross-link when both touch the same API.

---

## Product naming

**Execution plan:** [`plans/archive/v0-3-naming-migration.md`](plans/archive/v0-3-naming-migration.md) — consolidated discovery-through-closeout train for hard-cut naming (env, headers, binaries, paths, make namespace, layout, operator docs).
...
**Execution plan:** [`plans/archive/adminui-filesystem-dev-mode.md`](plans/archive/adminui-filesystem-dev-mode.md)
...
**Map to Chimera identity:** Older sketches derived collections from **user + project**. The target model is **tenant + project + optional flavor** and **base + flavor union** at retrieval time ([Workspace embedding scope (project + flavor)](version-v0.4.md#workspace-embedding-scope-project--flavor) in [`version-v0.4.md`](version-v0.4.md)). Any **manager + vectordb-cli** or pure-local indexer design must reconcile **collection naming** and **path** conventions with that model (and with **relative `source`** in HTTP ingest — see `[`plans/archive/indexer.md`](plans/archive/indexer.md)`) if both stacks coexist.
...
**Execution plan:** [`plans/archive/logs-ui-page-data-refreshing.md`](plans/archive/logs-ui-page-data-refreshing.md) — phased fix for `/ui/settings` summarized feed flicker, double-click card expansion, and admin form focus loss during SSE and admin poll rebuilds.
...
**Execution plan:** [`plans/archive/virtual-models-operator.md`](plans/archive/virtual-models-operator.md) — phased delivery for virtual models in operator SQLite, gateway runtime resolution, `/api/ui` CRUD, and `/ui/settings` cards.
...
Each row in **operator SQLite** (same store family as workspaces; see [`plans/archive/indexer-workspaces-sqlite-gateway-api.md`](plans/archive/indexer-workspaces-sqlite-gateway-api.md)):
...
| **routing.slug**          | Stable key for logs and metrics (aligns with [`plans/archive/operator-message-registry.md`](plans/archive/operator-message-registry.md)) |
...
- **Embedding model** panel: **combobox** of models from the live broker catalog (see [`plans/archive/indexer-embedding-model-and-workspace-purge.md`](plans/archive/indexer-embedding-model-and-workspace-purge.md)).
...
| Area                             | Quick check                                                                                                                                                              |
|----------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Product naming                   | README, onboarding, UI copy, and startup logs reflect Porcelain / Chimera / Locus decisions; [`plans/archive/v0-3-naming-migration.md`](plans/archive/v0-3-naming-migration.md) closed.  |
...
|--------------------------------------------|----------------------------------------------------------|--------|
| `[`plans/archive/indexer.md`](plans/archive/indexer.md)`     | Indexer milestones that may cross-link with this release | —      |
