...
This document plans a **separate operator-facing CLI** for health checks and BiFrost-oriented setup (Groq, Gemini, Ollama). It is distinct from the `chimera` gateway/supervisor binary built by `make chimera-build` (`chimera` / supervisor binaries via the root Makefile), to keep a small **read-only / config** tool separate from the long-running server.
...
- Ship [`config/chimera-broker.config.json`](../../config/chimera-broker.config.json) with **`config_store` enabled** so BiFrost persists provider config and exposes the management APIs this CLI uses.
...
**Also update** [`scripts/make-help.sh`](../../scripts/make-help.sh) and `.PHONY` list so `make help` documents the new targets.
...
- [ ] Enable `config_store` in [`config/chimera-broker.config.json`](../../config/chimera-broker.config.json); remove default-stack reliance on provider **environment variables**; refresh `env.example` and docs.
...
*Plan status: **draft for implementation** — v0.1 first; v0.8 adds layered config. Gateway defaults today: [`config/gateway.example.yaml`](../../config/gateway.example.yaml).*
