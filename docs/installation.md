...
You need **Go 1.22+**, **Node.js 20+**, **Git**, **GNU Make**, `gcc` or `clang` (CGO), plus `bash`, `curl`, and `tar` (and `unzip` on Windows/Git Bash for the Qdrant zip) for `make chimera-install` (or `make install`, which also runs `desktop-install`). Below: **Ubuntu**, **macOS**, and **Windows** for each.

Go drives `go build` / `make chimera-build` here and inside BiFrost’s `make build`. Node is only for building BiFrost (during `make chimera-install`); the `chimera` binary does not embed Node.
...
`make chimera-install` runs `scripts/install.sh`, which **sources** `scripts/install-gcc.sh` when neither `gcc` nor `clang` is on `PATH` (so any `PATH` fixups in that script apply to the same shell as BiFrost bootstrap). The helper uses the OS package manager when it can (`apt`, `dnf`/`yum`, `pacman`, `zypper`, `apk`, **Homebrew** on macOS, and on Windows **Git Bash / MSYS**: `winget` WinLibs packages, then **Chocolatey** `mingw`, then **Scoop** `mingw-winlibs`).

**Git Bash** sometimes fails `command -v gcc` even when `gcc.exe` is on `PATH`; `make chimera-install` uses a shared detector that also checks `gcc.exe` and common MinGW names, prefers the **UCRT** WinLibs layout over **MSVCRT** when both are present, and treats Chocolatey’s `gcc.exe` as present with `test -f` (not `-x`, which is unreliable for Windows binaries in MSYS).
...
On **Linux**, the helper runs `apt` / `dnf` / etc. with `sudo` when `sudo` is on `PATH`. Set `SKIP_AUTO_GCC=1` when running `make chimera-install` if you want to install a compiler yourself and avoid the helper (the install will fail until `gcc` or `clang` is available).
...
- **Windows:** Install a **MinGW-w64** toolchain so `gcc` is on `PATH` in the same shell you use for `make chimera-install` (e.g. **MSYS2**: `pacman -S mingw-w64-ucrt-x86_64-gcc`, then use that environment’s `bash` / `make`), or run `make chimera-install` from **WSL (Ubuntu)** with `build-essential`.
...
### `bash`, `curl`, and `tar` (for `make chimera-install`)
...
- **Windows:** **Git Bash** or **WSL** — install **GNU Make** (e.g. Chocolatey `choco install make`) and use `make chimera-install` from the same environment. **Git Bash** provides `bash` and `unzip` for the Qdrant zip path in `qdrant-from-release.sh`. **cmd.exe** alone is not supported for the bootstrap scripts.

### Windows and `make chimera-install`
...
```bash
git clone <your-fork-or-upstream-url> chimera-gateway
cd chimera-gateway
```

Use whichever remote you develop against; the install steps are the same.

## Install BiFrost and Qdrant (`make chimera-install`)

`make install` runs `make chimera-install` and then `make locus-desktop-install` (OS packages for the desktop WebView build). This section describes `chimera-install` only — use it alone on headless hosts when you only need the toolchain and `./bin/` binaries.

Run from the repository root (idempotent — safe to repeat after `chimera/deps.lock` changes or partial failures):
...
2. Runs `scripts/install-bootstrap.sh`, which:
   ...
   - Creates `.deps/qdrant` if needed, **clones** [github.com/qdrant/qdrant](https://github.com/qdrant/qdrant), and **checks out** `QDRANT_RELEASE` from `chimera/deps.lock` (same pin as the prebuilt binary).
...
| `git` / `curl` / `make` / `bash` not found | See prerequisites; on Windows use **GNU Make** + **Git Bash** for `make chimera-install`. |
| BiFrost `make` errors | Network or incomplete clone; try `make clean-all CONFIRM=1` then `make chimera-install`, or check BiFrost docs. |
| Qdrant fetch fails | Ensure `unzip` (Windows) or `tar` (Unix) on PATH; confirm `QDRANT_RELEASE` in `chimera/deps.lock`. |

To refresh **Qdrant only** (no BiFrost): `bash scripts/qdrant-from-release.sh` from the repo root (same script `make chimera-install` runs).

## Build the `chimera` binary

After dependencies are in place (BiFrost is only required when you run a supervised stack or point the gateway at a live upstream — not strictly required **only** to compile `chimera`):
...
For day-to-day work on the operator settings UI (`embedui/`), you can serve static assets from the repo tree instead of rebuilding `chimera-gateway` after every JavaScript or CSS change.
...
Details: [`plans/archive/adminui-filesystem-dev-mode.md`](plans/archive/adminui-filesystem-dev-mode.md) and [`embedui/settings/README.md`](../chimera/chimera-gateway/internal/server/adminui/embed/embedui/settings/README.md).
