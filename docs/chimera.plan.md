...
**chimera-gateway** is the thin orchestrator clients talk to: a **single OpenAI-compatible** entrypoint (for example Continue and similar tools) so operators do not juggle manual model switching in the IDE for every turn. Clients authenticate with a **gateway-issued API token** that binds the **tenant** and, when retrieval is enabled, **scopes memory** so embeddings and retrieved context apply only to that tenant’s data.
...
|-----------|------|----------------------|-------------|---------------------|
| **Gateway** | Routing + **virtual `chimera-<semver>`** + **fallback chain** + **structured logs**; **RAG**, vector store, **indexer REST** when enabled; **ensembles** + escalation when implemented | **BiFrost** (HTTP: chat; **embeddings** for RAG); **vector store** when RAG on; **gateway token YAML** | **Continue**, indexers, HTTP clients | `GET /v1/models`, `/v1/chat/completions`, `GET /health`; when RAG: `POST /v1/ingest`, `GET /v1/indexer/config`, storage endpoints, RAG headers |
| **BiFrost** | Multi-provider OpenAI-compatible proxy | Provider accounts & keys; optional **peer upstream** | chimera-gateway | HTTP OpenAI-compatible `/v1` (chat + embeddings as configured) |
...
| **Qdrant** | Vector persistence for gateway-mediated RAG | Persistent storage / ops backup policy | **chimera-gateway**; direct writes allowed anytime | HTTP REST / gRPC (per backend) |
| **Indexer** | Workspace **watch/index**: configured roots, ignore rules, hashing; **`POST /v1/ingest`** or **chunked ingest session** when files exceed gateway limits; gateway **chunks, embeds, writes vectors**—no local embeddings | **chimera-gateway** (`GET /v1/indexer/config`, ingest + optional storage/corpus paths); **`CHIMERA_GATEWAY_TOKEN`**; YAML config layers; optional **`chimera serve`/desktop** supervision (`indexer.supervised.*`) | **Operators** (**standalone** **`chimera-index`** or supervised **child**); serves **gateway** RAG corpus **via** ingest | HTTP **client** → gateway: `GET /v1/indexer/config`, `POST /v1/ingest`, `/v1/ingest/session` (+ chunk/complete templates); optional `GET /v1/indexer/storage/health`, `GET /v1/indexer/storage/stats`, `GET /v1/indexer/corpus/inventory` |
...
|---------|--------|----------------------|
| **v0.1** | **Portable Go gateway** in front of **BiFrost** (OpenAI-compatible upstream): virtual `chimera-<semver>` model on `GET /v1/models`, **routing + fallback chain**, **SSE streaming**, gateway tokens (**YAML**, **mtime** reload), `GET /health` probing the configured **upstream**, **structured logging**, operator **`docs/`** (overview, network/install, configuration reference), **`vscode-continue/`** samples. **Single-operator focus.** Optional **desktop** shell and **`chimera serve`** supervision of BiFrost/Qdrant for local ergonomics. **No** gateway-mediated RAG APIs yet. **MCP** only via the IDE until gateway-native MCP lands later. | [version-v0.1.md](version-v0.1.md) |
...
Deliberate choices so operators can run Chimera **without** mandatory containers or heavy infrastructure.
...
3. **Supervised stack** — **`chimera serve`** **may** supervise **BiFrost** and **Qdrant** as **child processes** so one operator command can bring up a working local stack.
...
3. **Continue samples** — Directory `vscode-continue/`: `README.md` and example config showing `apiBase`, `apiKey`, model selection from `GET /v1/models`, and custom headers when RAG applies (`X-Chimera-Project`, `X-Chimera-Flavor-Id`).
...
1. **Virtual `chimera-<semver>` model** — Gateway **prepends** a **virtual** entry on `GET /v1/models`: `id` = `Chimera-` + **gateway semantic version** (must match what clients send as `model`). That path receives **routing policy**, **fallback chain**, and **RAG** when enabled; it is the **product** orchestration surface—not merely a catalog alias.

2. **Sequential fallback chain** — For `model: Chimera-<gateway_semver>`, use an **ordered list** of upstream model ids from **gateway configuration** (operator-maintained). On failure or **429**, try the **next** entry (**fail-fast**—*Resilience · 2*).
...
1. **Product name** — Use **chimera-gateway** as the canonical name for the entrypoint in docs, config, and operator runbooks (orchestrator / router are fine as synonyms where helpful).
...
4. **Orchestrated vs explicit model choice** — Clients choose the **virtual** `chimera-<semver>` id for gateway-orchestrated turns (*Gateway turn orchestration · 1*) or an **explicit** upstream id for direct proxy (*Compatibility · 2*).
...
2. **Gateway responsibilities** — When `model: chimera-<semver>`, apply **routing policy** and **configured fallback chain** (*Gateway turn orchestration*). When **RAG** is enabled, decide **if and what** to retrieve. For **ensembles**, decide **when** to run phases, **critique/synthesize**, escalation, paste-back merge.
...
1. **Documented operator path** — Document **`chimera`**, **`chimera serve`**, and **Makefile** flows (*Portability · 3–4*).
...
5. **Single-document bootstrap** — Prefer **one** obvious getting-started path in README/Makefile so `make …` / `chimera serve` brings up the documented stack.
...
4. **Operations commands** — **`make`**, **`chimera`**, logs, rebuild, **Ops** helpers.
...
2. **`GET /v1/indexer/config`** — `Authorization: Bearer <gateway token>` (**same** as chat). JSON includes **effective** `chunk_size`, `chunk_overlap`, `embedding_model`, `ingest_method` + `ingest_path`, required/optional headers (`X-Chimera-Project`, `X-Chimera-Flavor-Id`), minimum payload fields, collection naming summary, `gateway_version`, and **running** indexer-relevant knobs.
...
8. **`X-Chimera-Project`** — On chat/completions (when **RAG** applies) and on **ingestion**; falls back to token default (*Tenant authentication · 2*).

9. **`X-Chimera-Flavor-Id`** — Optional corpus selector within tenant+project—see *Workspace indexing · 10*.
...
2. **Project scope on the wire** — Resolve `project_id` from `X-Chimera-Project` or token default; allowlists for unknown projects. `X-Chimera-Flavor-Id` selects corpus (*Workspace indexing · 8–10*).
...
1. **Long-lived service** — The gateway runs continuously while Chimera is in use; no process restart between ordinary user messages.
...
1. **Independent stacks, independent tenants** — Each **chimera-gateway** is the sole **client-facing** entrypoint on **their** machine; separate tokens, vector partitions, policy.
...
1. **Rules and heuristics first** — For `chimera-<semver>`, combine heuristics with **fallback chain**—not an LLM every turn. **Explicit** ids → **direct proxy**.
...
2. **Ensemble triggers** — **Automatic** + manual `//deep` (trimmed); only for **virtual `chimera-<semver>`**; gateway **may** strip `//deep` upstream. `N` follows *Ensemble orchestration · 1*.
