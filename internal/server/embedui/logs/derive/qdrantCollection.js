 ...
 *
 * Requires ClaudiaLogs.sha1Hex from derive/sha1.js (emn178/js-sha1 MIT).
 */
...
function qdrantCollectionName(tenantId, projectId, flavorId) {
  if (!ClaudiaLogs || !ClaudiaLogs.sha1Hex) return "";
  ...
  var hex = ClaudiaLogs.sha1Hex(key);
  var suffix = String(hex).slice(0, 8);
  var full = "claudia-" + prefix + "-" + suffix;
...
/** Counters + summary KV use only log lines at or after the last qdrant.version (Qdrant process restart). */
...
function qdrantOperatorLine(flat, resolveColl) {
  ...
  switch (msg) {
    case "qdrant.startup.banner":
      return "Starting up …";
    case "qdrant.version":
      return "Vector store: Qdrant " + (ver || "").trim();
    ...
    default:
      if (msg.indexOf("qdrant.") === 0) {
        ...
        var slugRest = String(flat.msg || msg || "")
          ...
          .replace(/\./g, " ")
          .trim();
        return slugRest ? "Qdrant · " + slugRest : "Qdrant event";
      }
...
globalThis.ClaudiaLogs.Derive.qdrantSlug = qdrantSlug;
globalThis.ClaudiaLogs.Derive.qdrantCollectionName = qdrantCollectionName;
globalThis.ClaudiaLogs.Derive.qdrantCollectionNameFromIndexerMeta = qdrantCollectionNameFromIndexerMeta;
globalThis.ClaudiaLogs.Derive.qdrantSliceCurrentProcess = qdrantSliceCurrentProcess;
globalThis.ClaudiaLogs.Derive.qdrantCollectionDisplay = qdrantCollectionDisplay;
globalThis.ClaudiaLogs.Derive.qdrantOperatorLine = qdrantOperatorLine;
globalThis.ClaudiaLogs.Derive.qdrantIndexerCollectionStatusLabel = qdrantIndexerCollectionStatusLabel;
globalThis.ClaudiaLogs.Derive.qdrantCardModel = qdrantCardModel;
